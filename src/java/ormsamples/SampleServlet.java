	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (action.equals("create")) {
				...
				ORMDatabaseInitiator.dropSchema(modelo.orm.MapeoSpotofyPersistentManager.instance());
			}
			else {
		...
		//	<filter-class>ormsamples.MapeoSpotofyFilter</filter-class>
		//</filter>
		//
		//<filter-mapping>
		//	<filter-name>ORMFilter</filter-name>
		...
		//
		//modelo.orm.MapeoSpotofyPersistentManager.instance().getSession().close();", true);
